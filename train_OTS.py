...
def transmission_estimate_test(im, A, sz):
    im3 = np.empty(im.shape, im.dtype)

    for ind in range(0, 3):
        im3[:, :, ind] = im[:, :, ind]/A[ind]

    transmission = 1 - HAZE_WEIGHT_test*dark_channel(im3, sz)
    return transmission
...
if __name__ == '__main__':
    ...
    train_dir = 'trainset'
    test_dir = 'testset'
    im_dir_train = os.path.join(data_dir, in_or_out, train_dir, 'OTS')
    ref_dir_train = os.path.join(data_dir, in_or_out, train_dir, 'clear_images')
    im_dir_test = os.path.join(data_dir, in_or_out, test_dir, 'hazy')
    ref_dir_test = os.path.join(data_dir, in_or_out, test_dir, 'gt')
    ...
    for fn in os.listdir(im_dir_train):
        ...
        w1, w2, w3, b = propagate(w1, w2, w3, b, img, ref_img, t_refined, A,  learning_rate = 0.25)
    ...
    for x in os.listdir(im_dir_test):
        ...
        t_estimated = transmission_estimate_test(img_test, A, PATCH_SIZE)
